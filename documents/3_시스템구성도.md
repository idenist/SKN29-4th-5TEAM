...
```mermaid
flowchart TD
    User[사용자 브라우저] -->|HTTP 80| EC2[AWS EC2<br/>52.78.46.170]
    EC2 --> Nginx[Nginx Container<br/>80:80]

    subgraph FE[Frontend 영역 - React/Vite]
        ReactBuild[frontend/dist<br/>React 빌드 결과물]
        AuthUI[로그인/회원가입 UI]
        ProfileUI[프로필/마이페이지 UI]
        PolicyUI[정책 검색/추천 카드 UI]
        ChatUI[AI 챗봇 UI]
        CommunityUI[커뮤니티 UI]
        Responsive[Flex/Grid 반응형 레이아웃]
        LoadingUX[로딩/실패/재시도 UX]
    end

    Nginx -->|/ 정적 파일 서빙| ReactBuild
    ReactBuild --> AuthUI
    ReactBuild --> ProfileUI
    ReactBuild --> PolicyUI
    ReactBuild --> ChatUI
    ReactBuild --> CommunityUI
    PolicyUI --> Responsive
    ChatUI --> LoadingUX

    Nginx -->|/api/ reverse proxy| Backend[Backend Container<br/>Gunicorn + Django<br/>internal 8000]
    Nginx -->|/admin/ reverse proxy| Backend
    Nginx -->|/static/ 정적 파일| StaticVol[(static_volume<br/>Django staticfiles)]

    subgraph BE[Backend 영역 - Django REST API]
        Django[Django REST Framework]
        AuthAPI[Auth/User API<br/>/api/auth/]
        PolicyAPI[Policy API<br/>/api/policies/]
        AIAPI[AI Chat API<br/>/api/ai/chat/]
        CommunityAPI[Community API<br/>/api/community/]
        NotificationAPI[Notification API<br/>/api/notifications/]
        MyPageAPI[MyPage API<br/>/api/mypage/]
        UploadAPI[Upload API<br/>/api/uploads/]
        Common[공통 응답/예외 처리]
    end
    ...
    Django --> AIAPI
    Django --> CommunityAPI
    Django --> NotificationAPI
    Django --> MyPageAPI
    Django --> UploadAPI
    Django --> Common

    subgraph AWS[AWS Managed Resources]
        RDS[(AWS RDS PostgreSQL<br/>DB: youth_search<br/>5432)]
        S3[(AWS S3<br/>skn29-youth-profile-images<br/>프로필 이미지 저장소)]
        EmailSMTP[Gmail SMTP<br/>이메일 발송]
        NaverAPI[Naver Search API<br/>뉴스/검색 확장 optional]
        OpenAI[OpenAI API<br/>Embedding / LLM]
        Tavily[Tavily 또는 공식 외부 검색 API<br/>조건부 사용]
    end
    ...
    NotificationAPI --> RDS
    MyPageAPI --> RDS
    UploadAPI -->|프로필 이미지 업로드/삭제| S3
    NotificationAPI -.이메일 알림/인증 메일 확장.-> EmailSMTP
    Django -.뉴스/검색 확장 기능.-> NaverAPI

    subgraph RAG[RAG / LLM 영역]
        AIAPI --> Condition[Condition Extractor<br/>나이/지역/관심분야 추출]
        Condition --> Router[Router<br/>도메인/source_category 판단]
        Router --> Retriever[Internal Retriever]
        Retriever --> Chroma[(Chroma Vector DB<br/>/app/data/vector_db)]
        Retriever --> GraphRetriever[Graph Retriever<br/>선택적 사용]
        GraphRetriever -.환경변수 설정 시.-> Neo4j[(Neo4j Graph DB<br/>optional)]
        Chroma --> HybridMerge[Vector + Graph Hybrid Merge]
        Neo4j -.optional.-> HybridMerge
        HybridMerge --> PostFilter[검색 결과 후처리<br/>마감/지역/도메인/출처 필터]
        PostFilter --> Sufficiency[검색 결과 충분성 검사]
        Sufficiency -->|충분| Eligibility[신청 가능성 판단]
        Sufficiency -->|부족| External[공식 외부 출처 검색 또는 안내 Fallback]
        External -.조건부.-> Tavily
        External --> Eligibility
        Eligibility --> Benefit[개인화 근거/예상 혜택 계산]
        Benefit --> Answer[Answer Generator]
        Answer -->|추천 결과 있음| RuleBased[Rule-based 짧은 요약]
        Answer -->|LLM 필요 시| OpenAI
        RuleBased --> AIResponse[answer + recommendations + meta 반환]
        OpenAI --> AIResponse
    end
    ...
    subgraph Docker[Docker Compose Project<br/>skn29-4th-5team]
        Nginx
        Backend
        DBContainer[PostgreSQL Container<br/>db:5432<br/>healthcheck 통과]
        StaticVol
        FrontDist[(frontend/dist bind mount)]
        ChromaVol[(data/vector_db bind mount<br/>EC2 host → backend container)]
        PostgresVol[(postgres_data volume)]
    end

    ReactBuild --- FrontDist
    Backend --- ChromaVol
    DBContainer --- PostgresVol
    DBContainer -.compose에는 포함<br/>운영 연결은 RDS 기준.-> RDS
```
...
```mermaid
flowchart TD
    User[사용자] -->|http://52.78.46.170/| EC2[AWS EC2]

    subgraph EC2Box[EC2 내부 Docker Compose]
        Nginx[Nginx Container<br/>80:80]
        Backend[Backend Container<br/>Gunicorn + Django<br/>expose 8000]
        DB[PostgreSQL Container<br/>db:5432]
        Static[(static_volume)]
        FrontendDist[(./frontend/dist<br/>React build bind mount)]
        VectorDB[(./data/vector_db<br/>Chroma bind mount)]
    ...
    Backend -->|collectstatic 결과 공유| Static
    Backend -->|CHROMA_PERSIST_DIR=/app/data/vector_db| VectorDB
    DB --> PgData

    subgraph AWSResources[AWS 외부 리소스]
        RDS[(AWS RDS PostgreSQL<br/>youth_search)]
        S3[(AWS S3<br/>skn29-youth-profile-images)]
        EmailSMTP[Gmail SMTP]
        NaverAPI[Naver Search API optional]
        OpenAI[OpenAI API]
    end

    Backend -->|DB_HOST = RDS endpoint| RDS
    Backend -->|프로필 이미지 저장| S3
    Backend -.이메일 발송.-> EmailSMTP
    Backend -.뉴스/검색 확장.-> NaverAPI
    Backend -->|Embedding / LLM 호출| OpenAI

    DB -.docker-compose 포함<br/>실제 운영 DB 연결은 RDS로 판단.-> Backend
```
