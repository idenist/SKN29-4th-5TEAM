  ...
  return (
    ...
    >
      <div className="home-policy-grid">
        {policies.map((policy) => (
          <Link
            key={policy.itemId || policy.id}
            to={`/policies/${policy.itemId || policy.id}`}
            className="home-policy-link"
          >
            <Card interactive className="home-policy-card">
              ...
              <h3>{policy.title}</h3>
              <p>{policy.summary || policy.description}</p>
