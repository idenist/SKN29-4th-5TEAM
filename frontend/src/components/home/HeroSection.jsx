...
export default function HeroSection() {
  ...
  const toChatQuestion = (question) => `/chat?message=${encodeURIComponent(question)}`;

  const handleSubmit = (event) => {
    event.preventDefault();
    const nextKeyword = keyword.trim();
    navigate(nextKeyword ? toChatQuestion(nextKeyword) : '/chat');
  };
  ...
  return (
    <section className="home-hero" aria-labelledby="home-hero-title">
      <div className="home-hero-inner">
        ...
        <form className="home-search-panel" role="search" aria-label="AI 안쉼 챗봇 질문" onSubmit={handleSubmit}>
          ...
          <div className="home-search-row">
            ...
            />
            <button type="submit">AI 안쉼에게 묻기</button>
          </div>
        </form>
        <PopularSearchSection />
        <div className="home-suggestion-list" aria-label="추천 질문">
          {suggestions.map((suggestion) => (
            <Link key={suggestion} to={toChatQuestion(suggestion)} className="home-suggestion-chip">
              {suggestion}
            </Link>
          ))}
        </div>
